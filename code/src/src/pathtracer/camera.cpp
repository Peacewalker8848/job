...
Ray Camera::generate_ray(double x, double y) const {
  ...
  // x, y in [0,1] where (0,0) is bottom-left of the image.
    double hFovRad = hFov * M_PI / 180.0;
    double vFovRad = vFov * M_PI / 180.0;

    // Map x,y to camera‐space coordinates
    double px = (2.0 * x - 1.0) * tan(hFovRad * 0.5);
    double py = (2.0 * y - 1.0) * tan(vFovRad * 0.5);

    // In camera space assume origin at (0,0,0) looking down -Z
    Vector3D dir_camera(px, py, -1.0);
